codeunit 50104 "Radio Show Management"
{
    ...
    begin
        ...
        PlaylistLineRec.Reset();
        PlaylistLineRec.SetRange("Playlist No.", PlaylistHeader."No.");
        if PlaylistLineRec.FindSet() then
            repeat
                RadioShowEntryRec.Init();
                RadioShowEntryRec."Entry No." := 0;
                ...
                RadioShowEntryRec."Royalty Cost" := PlaylistLineRec."Royalty Cost";
                RadioShowEntryRec."Advertising Revenue" := PlaylistLineRec."Advertising Revenue";
                RadioShowEntryRec.Insert();
            until PlaylistLineRec.Next() = 0;

        // Delete all Playlist Lines associated with the Playlist Header.
        // DeleteAssociatedPlaylistLines(PlaylistHeader."No."); Ao inserir o true na linha abaixo eu elimino a necessidade de escrever esta linha de eliminacao de linhas.

        // Delete the Playlist Header.
        PlaylistHeader.Delete(true);     //ao inserir o true estou ativando os triggers da headers que dizem para eliminar as linhas desta header.
    end;
    ...
    begin
        ...
        if RadioShowEntryRec.FindSet() then
            repeat
                ...
                SalesHeaderRec."No." := '';
                SalesHeaderRec.Validate("Sell-to Customer No.", RadioShowSetupRec."Ads Customer No.");
                SalesHeaderRec.Insert(true);   //Devo escrever true para chamar os triggers
