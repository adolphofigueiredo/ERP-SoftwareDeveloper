...
def process_produzione_cnc(file_path):
...
def clean_csv(csv_file):
    ...
    columns_to_validate = {
        "ID": 50,
        "ID_Produzione_CNC": 50,
        "ID_Pezzo_Produzione_Forgia_A_CNC": 50,
        "ID_Incidente_Macchina_Produzione_CNC": 50,
        "ID_Incidente_Operatore_Produzione_CNC": 50,
        "ID_Macchinario_Produzione_CNC": 50,
        "ID_Operatore_Produzione_CNC": 50,
    }
    ...
    df = df[df["Data_Ora_Produzione_CNC"].apply(validate_timestamp)]

    # Convalida che 'Numero_Pezzi_Ora' sia un intero positivo
    df = df[df["Numero_Pezzi_Ora"] > 0]
...
def insert_into_db(df):
    ...
    try:
        ...
        with open(temp_file.name, 'r') as f:
            copy_query = f"COPY public.produzione_cnc FROM STDIN WITH CSV"
